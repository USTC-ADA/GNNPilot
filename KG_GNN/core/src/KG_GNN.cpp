...
extern int64_t kg_gcn_spmm_mm_balance(
    torch::Tensor RowPtr,
    torch::Tensor ColIdx
);

extern int64_t kg_gcn_spmm_mm_balance_final(
    torch::Tensor RowPtr,
    torch::Tensor ColIdx);
...
extern void kg_gcn_spmm_mm_run_final(
    torch::Tensor RowPtr,
    torch::Tensor ColIdx,
    torch::Tensor input_a,
    torch::Tensor input_b,
...
TORCH_LIBRARY(KGGNN, m) {
    ...
    m.def("kg_nn_gcn_fused_run", &kg_nn_gcn_fused_run);
    m.def("kg_gcn_spmm_mm_balance", &kg_gcn_spmm_mm_balance);
    m.def("kg_gcn_spmm_mm_balance_final", &kg_gcn_spmm_mm_balance_final);
    m.def("kg_gcn_spmm_mm_run_final", &kg_gcn_spmm_mm_run_final);
